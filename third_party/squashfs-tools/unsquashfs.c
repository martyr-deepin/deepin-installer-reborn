...
int main(int argc, char *argv[])
{
	...
	for(i = 1; i < argc; i++) {
      ...
      if(++i == argc) {
        fprintf(stderr, "%s: -start missing start value\n",
          argv[0]);
        exit(1);
      }
      p_start = atoi(argv[i]);
      ...
      if(++i == argc) {
        fprintf(stderr, "%s: -end missing end value\n",
          argv[0]);
        exit(1);
      }
      p_end = atoi(argv[i]);
  ...
  if (p_start >= p_end) {
    fprintf(stderr, "%s: invalid -start and -end value\n",
      argv[0]);
    exit(1);
  }
...
options:
			...
			ERROR("\t\t\t\trather than use the default shell "
				"wildcard\n\t\t\t\texpansion (globbing)\n");
      ERROR("\t-start <value>\t\tset start value of progress "
        "default is 0\n");
      ERROR("\t-end <value>\t\tset end value of progress "
        "default is 100\n");
      ERROR("\t-quiet\t\t\tDisplay progress value only\n");
