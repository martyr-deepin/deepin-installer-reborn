...
TEST(ValidatePasswordTest, ValidatePassword) {
  ...
  if (need_lower_case) {
    EXPECT_EQ(ValidatePassword("ABC"), ValidatePasswordState::NoLowerCharError);
  }

  const bool need_upper_case = service::GetSettingsBool(
      service::kSystemInfoPasswordShallContainUpperCase);
  if (need_upper_case) {
    EXPECT_EQ(ValidatePassword("abc"), ValidatePasswordState::NoUpperCharError);
  }
  ...
  if (need_special_char) {
    EXPECT_EQ(ValidatePassword("abc"),
